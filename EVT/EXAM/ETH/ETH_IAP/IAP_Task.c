...
void IAP_EEPROM_READ( u32 StartAddr, u8 *Buffer, u32 Length )
{
    ...
    while(address < (StartAddr + Length))
    {
        *p_buff = (*(u32 *)address);
        ...
        /*********************************************************************
 * @fn      IAP_EEPROM_ERASE_UPDATE_AREA
 *
 ...
 * @return  none
 */
void IAP_EEPROM_ERASE_UPDATE_AREA(u32 StartAddr)
{
    u8 i = 0;
...
{
    u8 pageCnt = 0;
	u8 pBuff[RECE_BUF_LEN];
    u16 writableDataLen = 0, tempLen = 0, i = 0;
    u32 remainLen = 0, updateFlag;
    ...
    if(dataDeal.buffUsedLen){
        ...
        for(i = 0; i < (writableDataLen / 4); i++)          //Calculate checksum
...
#if defined(CH32F20x_D8C)
	IAP_EEPROM_ERASE_UPDATE_AREA(USER_IMAGE_START_ADD);
#elif defined(CH32F20x_D8W)
...
#if defined(CH32F20x_D8C)
	IAP_EEPROM_ERASE_UPDATE_AREA(BACKUP_IMAGE_START_ADD);
#elif defined(CH32F20x_D8W)
